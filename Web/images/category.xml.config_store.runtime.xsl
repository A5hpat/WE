...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:aspdnsf="urn:aspdnsf" exclude-result-prefixes="aspdnsf">
  ...
  <xsl:template match="/">
    <div class="navbar navbar-default topnav" role="navigation">
      <ul class="nav nav-pills navbar-left">
        <li>
          <button type="button" class="btn btn-default" data-toggle="modal" data-target="#headerentity">
            <xsl:value-of select="$EntitySETitle" />
          </button>
        </li>
        <li>
          <button type="button" class="btn btn-default" data-toggle="modal" data-target="#filterProducts">
            <xsl:text>Filter</xsl:text>
          </button>
        </li>
        <li>
          <button type="button" class="btn btn-default" data-toggle="modal" data-target="#sortProducts">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <!--  add CatID to this xml package  -->
        <xsl:value-of select="aspdnsf:XmlPackage('guidednavigation.category.xml.config', concat('CatID=', Categoryid) )" disable-output-escaping="yes" />
        <!--<xsl:value-of select="aspdnsf:XmlPackage('guidednavigationcategory.xml.config', concat('CatID=', Categoryid) )" disable-output-escaping="yes" />-->
      ...
      <div class="row">
        <a class="thumbnail">
          <xsl:attribute name="href">
            <xsl:value-of select="concat('http://localhost/nalpac4/c-', $subcatid, '.aspx')"></xsl:value-of>
          </xsl:attribute>
          <img>
            <xsl:attribute name="src">
              <xsl:value-of select="concat('http://localhost/nalpac4/images/category/icon/', $subcatid, '.jpg')"></xsl:value-of>
            </xsl:attribute>
          </img>
        </a>
      </div>
  ...
  <xsl:template name="ProductCell">
    ...
    <xsl:variable name="plink" select="concat('http://localhost/nalpac4/p-', /root/Products/Product/ProductID, '-', /root/Products/Product/SEName, '.aspx')"></xsl:variable>
    ...
    <a>
      <xsl:attribute name="href">
        <xsl:value-of select="concat('http://localhost/nalpac4/p-', ProductID, '-', SEName, '.aspx')" />
      </xsl:attribute>
      ...
      <img class="product-image img-responsive">
        ...
        <xsl:attribute name="onerror">
          <xsl:text>this.src='http://localhost/nalpac4/images/image-not-found.png'</xsl:text>
        </xsl:attribute>
      </img>
    </a>
    <div class="caption">
      ...
      <div class="guidedNavNameWrap">
        ...
        <a>
          <xsl:attribute name="href">
            <xsl:value-of select="concat('http://localhost/nalpac4/m-', manufacturerid, '-', msename, '.aspx')" />
          </xsl:attribute>
          <xsl:attribute name="class">
      ...
      <div class="entity-product-price-wrap grid-item-price-wrap">
        <xsl:value-of select="aspdnsf:GetVariantPrice(VariantID, '0', '0', '0', '0', '0', 'for sale', '0')" disable-output-escaping="yes" />
      </div>
      <div>
        <input>
            ...
            <div class="modal-content">
              ...
              <div class="model-body">
                <p>
                  ...
                  <br />
                  <span>
                        Your Price <xsl:value-of select="format-number($CustomerSalePrice, '#.00')" disable-output-escaping="yes"></xsl:value-of></span>
                </p>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <a class="btn btn-default" href="http://localhost/nalpac4/shoppingcart.aspx">
        ...
        <div id="login-mod" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myLogginModalLabel">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <a class="btn btn-default" href="http://localhost/nalpac4/signin.aspx">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <script type="text/javascript">
                ...
                url: 'http://localhost/nalpac4/nal_addtocart.aspx/nalpac_AddToCart',
