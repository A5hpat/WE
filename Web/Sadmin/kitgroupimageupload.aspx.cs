...
{
	public partial class KitGroupImageUpload : AspDotNetStorefront.Admin.AdminPageBase
	{
		...
		protected void btnUpload_Click(object sender, EventArgs e)
		{
			...
			foreach (RepeaterItem item in rptItemImages.Items)
			{
					...
					item.ItemType == ListItemType.AlternatingItem)
				{
					...
					if (kitItem != null)
					{
						var flpKitItem = item.FindControl<FileUpload>("flpKitItem");
						if (flpKitItem.HasFile)
							kitItem.UploadImage(new HttpPostedFileWrapper(flpKitItem.PostedFile));
					}
				}
			}
			BindData();
