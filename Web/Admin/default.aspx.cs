	...
	public partial class _default : AspDotNetStorefront.Admin.AdminPageBase
	{
		...
		void CheckForSecurityAlerts()
		{
			...
			AlertMessageDisplay.PushAlertMessage(
					string.Format("You have {0} security issue(s) and {1} configuration issue(s) to address.  Click <a href=\"about.aspx\">here</a> to find out more.",
		...
		void CheckForMaintenanceAlert()
		{
			...
			if(DateTime.TryParse(AppLogic.AppConfig("NextMaintenanceDate"), out nextMaintenanceDate))
			{
				if(nextMaintenanceDate < DateTime.Now)
					AlertMessageDisplay.PushAlertMessage("It has been more than 1 month since <a href=\"monthlymaintenance.aspx\">Monthly Maintenance</a> was run on your site.  Not performing this maintenance regularly can lead to slow site performance and possible support issues.", AlertMessage.AlertType.Warning);
            }
		}
