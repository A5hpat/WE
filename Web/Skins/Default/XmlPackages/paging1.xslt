    ...
    <div class="productControl">
      <div class="entityPageSortWrap">
		<xsl:if test="$PageCount &gt; 1">
			<xsl:attribute  name="class">entityPageSortWrap row</xsl:attribute>
		</xsl:if>
		<div>
			<xsl:if test="$PageCount &gt; 1">
		...
		<xsl:if test="$PageCount &gt; 1">
			<div class="paging col-md-6">
				<xsl:if test="$DisplayDescriptivePageNumber = true()">
					<span class="viewingPageHeading productcontrollabel">
				...
				<div class="pageLinksWrap btn-group" >
					<xsl:if test="$CurrentPage &gt; 1 and ($DisplayFirstAndLastPageButtons = true() or $DisplayNextAndPreviousPageButtons = true())">
						<xsl:if test="$DisplayFirstAndLastPageButtons = true()">
							<button type="button" class="btn btn-default pagelink firstLink" onclick="setParam('pagenum', '{1}');">
								<xsl:value-of select="$FirstPageButtonContent" />
							</button >
						</xsl:if>
						<xsl:if test="$DisplayNextAndPreviousPageButtons = true()">
							<button type="button" class="btn btn-default pagelink prevLink" onclick="setParam('pagenum', '{$CurrentPage - 1}');">
					...
					<xsl:if test="$CurrentPage &lt; $PageCount and ($DisplayFirstAndLastPageButtons = true() or $DisplayNextAndPreviousPageButtons = true())">
						<xsl:if test="$DisplayNextAndPreviousPageButtons = true()">
							<button type="button" class="btn btn-default pagelink nextLink" onclick="setParam('pagenum', '{$CurrentPage + 1}');">
								<xsl:value-of select="$NextPageButtonContent" />
							</button>
						</xsl:if>
						<xsl:if test="$DisplayFirstAndLastPageButtons = true()">
							<button type="button" class="btn btn-default pagelink lastLink" onclick="setParam('pagenum', '{$PageCount}');">
    ...
    <xsl:if test="$page &gt; 0 and $page &lt;= $CurrentPage + $ForwardPages">
      <xsl:choose>
        <xsl:when test="$page = $CurrentPage">
			<button type="button" class="btn btn-default btn-primary currentpage pagelink">
				<xsl:value-of select="$page" />
			</button>
        </xsl:when>
        <xsl:otherwise>
			<button type="button" class="btn btn-default pagelink" onclick="setParam('pagenum', '{$page}');">
				<xsl:value-of select="$page" />
			</button>
        </xsl:otherwise>
      </xsl:choose>
