...
namespace AspDotNetStorefrontWSI
{
	...
	public class WSI
    {
        ...
        public enum OrderManagementActionEnum
        {
            ...
            MarkAsShipped, SetPrivateNotes, SetOrderWeight, SetCustomerServiceNotes, SendToFedexShippingMgr,
            ...
            get
            {
                if (m_XmlTableSpecs == null)
                {
                    try
                    {
                        ...
                        m_XmlTableSpecs.Load(CommonLogic.SafeMapPath(CommonLogic.IIF(AppLogic.IsAdminSite, "~/", string.Empty) + "ipx.xml"));
                    }
                    catch
                    {
        ...
        public bool AuthenticateRequest(string email, string password)
        {
            ...
            m_ThisCustomer = new Customer(null, email, true, false, true);
            ...
            if (m_ThisCustomer.IsRegistered && m_ThisCustomer.IsAdminUser)
            {
				...
				{
					loginAllowed = m_ThisCustomer.CheckLogin(password);
				}
            }
        ...
        public void DoIt(String LogFileName)
        {
            ...
            {
                String s = GetResultsAsString().Replace("utf-16", "utf-8").Replace("<?xml version=\"1.0\" encoding=\"utf-16\"?>", string.Empty); ;
                CommonLogic.WriteFile(CommonLogic.SafeMapPath(CommonLogic.IIF(AppLogic.IsAdminSite, "~/", string.Empty) + "images/" + LogFileName), s, true);
            }
        }
        ...
        private int LoadEntityTree(String EntityType, String XPathEntityName, int ParentEntityID, String EntityGUID, String OrigEntityName)
        {
            ...
            foreach (String s in XPathEntityName.Split('/'))
            {
                ...
                else
                {
                    ...
                    sql.Append(DB.SQuote(SearchEngineNameProvider.GenerateSeName(s)) + ",");
        ...
        public int LookupTableRecordByName(String TableName, String Name, bool HasDeletedFlag)
        {
            ...
            String sql = String.Format("select {0} from {1} with (NOLOCK) where {2} lower(Name)={3}", IDColumnName, TableName, CommonLogic.IIF(HasDeletedFlag, "deleted=0 and", string.Empty), DB.SQuote(Name.Trim().ToLowerInvariant()));
        ...
        private int SetupProductID(int ProductID, String ProductGUID, String ProductName, String SKU)
        {
            ...
            if (id == 0 && SKU.Length != 0)
            {
                ...
                try
                {
                    string query = "select ProductID from Product  with (NOLOCK)  where Deleted<>1 and lower(name)=" + DB.SQuote(ProductName) + CommonLogic.IIF(SKU.Trim().Length != 0, " and SKU=" + DB.SQuote(SKU), string.Empty);
                    if (m_DBTrans != null)
                    {
                        // if a transaction was passed, we should use the transaction objects connection
        ...
        private int SetupVariantID(int ProductID, int VariantID, String VariantGUID, String VariantName, String SKUSuffix)
        {
            ...
            if (id == 0)
            {
                ...
                try
                {
                    string query = "select VariantID from ProductVariant  with (NOLOCK)  where Deleted=0 and lower(name)=" + DB.SQuote(VariantName) + "and ProductID=" + ProductID + CommonLogic.IIF(SKUSuffix.Trim().Length != 0, " and SKUSuffix=" + DB.SQuote(SKUSuffix), string.Empty);
                    if (m_DBTrans != null)
                    {
                        // if a transaction was passed, we should use the transaction objects connection
                        rsp = DB.GetRS(query, m_DBTrans);
        ...
        private int SetupNodeID(int nodeID, String nodeGUID, String NodeType)
        {
            int id = nodeID;
        ...
        private int SetupEntityNodeID(int nodeID, String nodeGUID, String entityName)
        {
            int id = nodeID;
            if (id == 0 && nodeGUID.Length == 0)
            {
            ...
            try
            {
                switch (NodeAction)
                {
                    ...
                    case NodeActionEnum.Update:
                        {
                            ProductTypeID = SetupNodeID(ProductTypeID, ProductTypeGUID, NodeType);
        ...
        private int ProcessAppConfig(XmlNode node)
        {
			...
			var existingConfigIsSuperOnly = false;
            ...
            try
            {
                switch (NodeAction)
                {
                    case NodeActionEnum.Add:
                        {
                            ...
                            else
                            {
                                ...
                                if(AppConfigSuperOnly)
                                    Security.LogEvent(
                                        "WSI Setting Created Success",
                                        "Parameter Name: " + AppConfigName,
                                        0,
                                        m_ThisCustomer.CustomerID,
                                        m_ThisCustomer.CurrentSessionID);
                    ...
                    case NodeActionEnum.Update:
                        {
                            AppConfigID = SetupNodeID(AppConfigID, AppConfigGUID, NodeType);
                                ...
                                if (AppConfigID != 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        using (rsx)
                                        {
                                            if (rsx.Read())
                                            {
												existingConfigIsSuperOnly = DB.RSFieldBool(rsx, "SuperOnly");
                                                if (existingConfigIsSuperOnly 
													&& !m_ThisCustomer.IsAdminSuperUser
													&& m_DBTrans != null)
                                                {
													if(existingConfigIsSuperOnly)
														Security.LogEvent(
															"WSI Setting Change Failed",
															"Parameter Name: " + AppConfigName,
															0,
															m_ThisCustomer.CustomerID,
															m_ThisCustomer.CurrentSessionID);
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    using (rsx)
                                    {
										...
										{
											existingConfigIsSuperOnly = DB.RSFieldBool(rsx, "SuperOnly");
											if (existingConfigIsSuperOnly && !m_ThisCustomer.IsAdminSuperUser)
                                            {
                                                if (m_DBTrans != null)
                                                {
                                                    if(existingConfigIsSuperOnly)
                                                        Security.LogEvent(
                                                            "WSI Setting Delete Failed",
                                                            "Parameter Name: " + AppConfigName,
                                                            0,
                                                            m_ThisCustomer.CustomerID,
                                                            m_ThisCustomer.CurrentSessionID);
        ...
        private void ProcessMapping(XmlNode node)
        {
            ...
            try
            {
                switch (NodeAction)
                {
                    case NodeActionEnum.Add:
                        {
                            EntityID = SetupEntityNodeID(EntityID, EntityGUID, EntityType);
                            ObjectID = SetupNodeID(ObjectID, ObjectGUID, "Product");
                            if (EntityID == 0)
                            {
        ...
        private void ProcessGetMappings(XmlNode node)
        {
            ...
            try
            {
                EntityID = SetupEntityNodeID(EntityID, EntityGUID, NodeType);
                if (EntityID == 0)
                {
                    Results.WriteErrorEntry("Required EntityID or EntityGUID not provided or Invalid");
                    if (m_DBTrans != null)
        ...
        private void GetImageData(StringBuilder tmpS, String EntityOrObjectName, String theSize, int ID)
        {
            ...
            if (ImageUrl.Length != 0 && ImageUrl.IndexOf("nopicture") == -1)
            {
                ...
                try
                {
                    tmpS.Append(Path.GetExtension(FN).ToLowerInvariant().Replace(".", string.Empty));
                }
                catch { }
                tmpS.Append("\"");
                tmpS.Append(" URL=\"");
                ...
                using (rs)
                {
                    if (rs.Read())
                    {
                        ...
                        if (IncludeImages)
                        {
                            ...
                            String[] m_ColorsSplit = new String[1] { string.Empty };
                                    ...
                                    for (int i = 1; i <= ig.MaxImageIndex; i++)
                                    {
                                        foreach (String c in m_ColorsSplit)
                                        {
                                            ...
                                            if (File.Exists(FN))
                                            {
                                                ...
                                                tmpS.Append(String.Format("<Img Index=\"{0}\" Color=\"{1}\" Extension=\"{2}\">{3}</Img>", i.ToString(), c, Path.GetExtension(FN).ToLowerInvariant().Replace(".", string.Empty), Base64Data));
                                            }
                                        }
                                    }
                                    tmpS.Append("</" + imagesize + ">");
        ...
        private void ProcessGetEntity(XmlNode node)
        {
            ...
            try
            {
                Results.WriteXml("<GetEntity EntityID=\"" + EntityID.ToString() + "\" EntityGUID=\"" + EntityGUID + "\" " + CommonLogic.IIF(XPathLookup.Length != 0, "XPathLookup=\"" + XmlCommon.XmlEncodeAttribute(XPathLookup) + "\"", string.Empty) + ">");
        ...
        private void ProcessGetProduct(XmlNode node)
        {
            ...
            {
                ForEntityID = SetupEntityNodeID(ForEntityID, ForEntityGUID, ForEntityType);
            }
                        ...
                        try
                        {
                            ...
                            using (rs)
                            {
                                while (rs.Read())
                                {
                                    ...
                                    String OutputXml = AppLogic.RunXmlPackage(PN, null, m_ThisCustomer, 1, string.Empty, xpnRuntimeParams, false, true);
        ...
        private void ProcessExecuteSQL(XmlNode node, int Idx)
        {
            ...
            try
            {
                ...
                if (sql.Length != 0)
                {
					...
					Security.LogEvent(
						"ExecuteSQL Node executed successfully",
        ...
        private void ProcessXmlPackage(XmlNode node)
        {
            String NodeType = node.Name;
            ...
            try
            {
                ...
                {
                    String s = AppLogic.RunXmlPackage(PackageName, null, null, 1, string.Empty, RuntimeParams, false, false); ;
        ...
        private void ProcessScanForXmlPackages(XmlNode node)
        {
            ...
            try
            {
                ...
                foreach (String s in PackageTypes)
                {
                    ...
                    {
						var xmlPackages = XmlPackageManager
							.GetXmlPackageInfosByPrefix(
								httpContext: new HttpContextWrapper(HttpContext.Current),
								searchLocations: XmlPackageLocation.Root | XmlPackageLocation.Skin,
								prefix: s,
								skinIds: new[] { SkinID })
        ...
        private void hlpEntityUpdate(EntitySpecs Specs, int EntityID, XmlNode node)
        {
            ...
            else if (XmlCommon.XmlField(node, "Name").Length != 0)
            {
                sql.Append(Separator);
                sql.Append("SEName=");
                sql.Append(DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(XmlCommon.XmlField(node, "Name")), 150)));
                Separator = ",";
            }
            ...
            foreach (String imagesize in "Icon,Medium,Large".Split(','))
            {
                ...
                if (nnx != null)
                {
                    ...
                    if (ImageData.Length != 0 && !DeleteFlag)
                    {
                        ...
                        if (!FN.EndsWith(".jpg", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".jpeg", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".gif", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".png", StringComparison.InvariantCultureIgnoreCase))
                        {
                            switch (ImageType.Replace(".", string.Empty).ToLowerInvariant())
                            {
                                case "gif":
                        ...
                        try
                        {
                            ...
                            if (img_Resize)
                            {
								...
								ImageResize.ResizeEntityOrObject(Specs.m_EntityName, TempImage, FN, eoID, imagesize, img_ContentType, img_Params, img_UseAppConfigs);
                        ...
                        {
							ImageResize.DisposeOfTempImage(TempImage);
                        }
                    }
                }
        ...
        private void hlpCustomerUpdate(int CustomerID, XmlNode node, bool AddingCustomer)
        {
            ...
            bool NewEmailAllowed = Customer.NewEmailPassesDuplicationRules(EMailField, c.CustomerID);
            ...
            if (Separator != String.Empty)
            {
                ...
                if(!string.IsNullOrEmpty(PWD))
                    Security.LogEvent(
                    "WSI Customer Password Update Success",
                    string.Empty,
                    CustomerID,
                    m_ThisCustomer.CustomerID,
                    m_ThisCustomer.CurrentSessionID);
        ...
        private void hlpProductUpdate(int ProductID, XmlNode node)
        {
            ...
            Separator = AddXmlFieldToSQLFieldBool("InventoryType/TrackInventoryBySize", "TrackInventoryBySize", node, sql, Separator, false);
            Separator = AddXmlFieldToSQLFieldBool("InventoryType/TrackInventoryByColor", "TrackInventoryByColor", node, sql, Separator, false);
                    ...
                    if (ImageData.Length != 0 && !DeleteFlag)
                    {
                        ...
                        try
                        {
                            ...
                            if (img_Resize)
                            {
								...
								ImageResize.ResizeEntityOrObject("Product", TempImage, FN, eoID, imagesize, img_ContentType, img_Params, img_UseAppConfigs);
            ...
            if (mx != null)
            {
                foreach (String imagesize in "Icon,Medium,Large".Split(','))
                {
                    ...
                    foreach (XmlNode nnx in mx.SelectNodes(imagesize + "/Img"))
                    {
                        ...
                        FN = Path.GetFileNameWithoutExtension(FN) + "_" + Index.ToString() + "_" + SafeColor + "." + Path.GetExtension(FN).ToLowerInvariant().Replace(".", string.Empty);
                        ...
                        if (ImageData.Length != 0 && !DeleteFlag)
                        {
                            ...
                            if (!FN.EndsWith(".jpg", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".jpeg", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".gif", StringComparison.InvariantCultureIgnoreCase) && !FN.EndsWith(".png", StringComparison.InvariantCultureIgnoreCase))
                            {
                                switch (ImageType.Replace(".", string.Empty))
                                {
                                    case "gif":
                                        TempImage = AppLogic.GetImagePath("Product", imagesize, true) + "tmp_" + FN + "gif";
                                        FN = AppLogic.GetImagePath("Product", imagesize, true) + FN + "gif";
                            ...
                            try
                            {
                                ...
                                if (img_Resize)
                                {
									...
									ImageResize.ResizeEntityOrObject("Product", TempImage, FN, eoID, imagesize, ContentType, img_Params, img_UseAppConfigs);
                    ...
                    else
                    {
                        ...
                        switch (MatchKey)
                        {
                            case InventoryMatchKeyEnum.SizeColor:
                                InventoryRecordAlreadyExists = (DB.GetSqlN("select count(*) as N from Inventory where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " lower(size)=" + DB.SQuote(CleanSize) + " and lower(color)=" + DB.SQuote(CleanColor), m_DBTrans) > 0);
                                break;
                            case InventoryMatchKeyEnum.VendorFullSKU:
                                InventoryRecordAlreadyExists = (DB.GetSqlN("select count(*) as N from Inventory where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " VendorFullSKU=" + DB.SQuote(VendorFullSKU), m_DBTrans) > 0);
                                break;
                            case InventoryMatchKeyEnum.VendorID:
                                InventoryRecordAlreadyExists = (DB.GetSqlN("select count(*) as N from Inventory where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " VendorID=" + DB.SQuote(VendorID), m_DBTrans) > 0);
                                break;
                        }
                        ...
                        else
                        {
                            ...
                            if (anyFound)
                            {
                                ...
                                {
                                    case InventoryMatchKeyEnum.SizeColor:
                                        sql2.Append(" where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " lower(size)=" + DB.SQuote(AppLogic.CleanSizeColorOption(Size).ToLowerInvariant()) + " and lower(color)=" + DB.SQuote(AppLogic.CleanSizeColorOption(Color).ToLowerInvariant()));
                                        break;
                                    case InventoryMatchKeyEnum.VendorFullSKU:
                                        sql2.Append(" where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " VendorFullSKU=" + DB.SQuote(VendorFullSKU));
                                        break;
                                    case InventoryMatchKeyEnum.VendorID:
                                        sql2.Append(" where " + CommonLogic.IIF(VariantID != 0, "VariantID=" + VariantID.ToString() + " and ", string.Empty) + " VendorID=" + DB.SQuote(VendorID));
                                        break;
                                }
                    ...
                    if (ImageData.Length != 0 && !DeleteFlag)
                    {
                        ...
                        try
                        {
                            ...
                            if (img_Resize)
                            {
								...
								ImageResize.ResizeEntityOrObject("Variant", TempImage, FN, eoID, imagesize, img_ContentType, img_Params, img_UseAppConfigs);
                    ...
                    case NodeActionEnum.Add:
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    sql.Append(DB.SQuote(SearchEngineNameProvider.GenerateSeName(EntityName)) + ",");
                                    sql.Append(ParentEntityID.ToString());
                                    sql.Append(")");
                                    sql = sql.Replace("^", Specs.m_EntityName);
                                    RunCommand(sql.ToString());
                    ...
                    case NodeActionEnum.Update:
                        {
                            ...
                            {
                                EntityID = SetupEntityNodeID(EntityID, EntityGUID, Specs.m_EntityName);
                            }
                            if (EntityID == 0 && EntityGUID.Length == 0)
                            {
                    ...
                    case NodeActionEnum.Add:
                        {
                            ...
                            else
                            {
                                ...
                                sql.Append(DB.SQuote(SearchEngineNameProvider.GenerateSeName(ProductName)) + ")");
                                RunCommand(sql.ToString());
                                Results.WriteVerboseEntry(NodeType + " OK");
        ...
        private int ProcessResetPassword(XmlNode node)
        {
            ...
            try
            {
                ...
                {
                    AppLogic.SendMail(AppLogic.AppConfig("StoreName") + " - Password Change", AppLogic.RunXmlPackage("notification.lostpassword.xml.config", null, TargetCustomer, TargetCustomer.SkinID, string.Empty, "thiscustomerid=" + TargetCustomer.CustomerID.ToString() + "&newpwd=" + p.ClearPassword, false, false), true, AppLogic.AppConfig("MailMe_FromAddress"), AppLogic.AppConfig("MailMe_FromName"), TargetCustomer.EMail, TargetCustomer.FullName(), string.Empty, string.Empty, AppLogic.MailServer());
                }
            ...
            try
            {
                ...
                switch (NodeAction)
                {
                    ...
                    case OrderManagementActionEnum.SendDistributorNotification:
                        Status = Gateway.OrderManagement_SendDistributorNotification(ord, false);
                        Results.WriteXml(String.Format(" Status=\"{0}\">", XmlCommon.XmlEncodeAttribute(Status)));
                        break;
                    ...
                    case NodeActionEnum.Add:
                        {
                            ...
                            else
                            {
                                ...
                                sql.Append(DB.SQuote(SearchEngineNameProvider.GenerateSeName(VariantName)) + ")");
                                RunCommand(sql.ToString());
                                Results.WriteVerboseEntry(NodeType + " OK");
