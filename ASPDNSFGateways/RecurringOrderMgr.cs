...
namespace AspDotNetStorefrontGateways.Processors
{
    ...
    public class RecurringOrderMgr
    {
        ...
        public String ProcessRecurringOrder(int OriginalRecurringOrderNumber, string XID, DateTime PaymentTime, out int NewOrderNumber)
        {
            ...
            if (cart.CartItems.Count != 0 && originalOrder.PaymentMethod != AppLogic.ro_PMPayPalExpress)
            {
                ...
                if (status == AppLogic.ro_OK)
                {
                    ...
                    try
                    {
                        AppLogic.SendOrderEMail(ProcessCustomer, OrderNumber, true, AppLogic.ro_PMCreditCard, false);
                    }
                    catch { }
                }
            }
        ...
        public String ProcessAutoBillDeclined(int OriginalRecurringOrderNumber, String XID, DateTime PaymentTime, String RecurringSubscriptionID, String Reason)
        {
            ...
            {
                // send email notification to customer
                string emailSubject = String.Format("{0} - Recurring Order Declined", AppLogic.AppConfig("StoreName"));
                string emailBody = String.Format("Your recurring order payment was declined. The original order number was: {0}. The SubscriptionID is: {1}. Please update your billing information on the account page to correct. Thanks", OriginalRecurringOrderNumber.ToString(), RecurringSubscriptionID);
                AppLogic.SendMail(subject: emailSubject,
					body: emailBody + AppLogic.AppConfig("MailFooter"),
					useHtml: true,
					fromAddress: AppLogic.AppConfig("ReceiptEMailFrom"),
        ...
        public String ProcessAutoBillFullRefund(int OriginalRecurringOrderNumber)
        {
            ...
            String Status = Gateway.OrderManagement_DoFullRefund(ord, "AutoBill Recurring Order Canceled");
            if (Status == AppLogic.ro_OK)
            {
                Status = ProcessAutoBillCancel(OriginalRecurringOrderNumber);
            }
