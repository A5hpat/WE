...
namespace AspDotNetStorefrontCore
{
	...
	public class KitProductData
	{
		...
		private List<KitGroupData> m_groups;

		/// <summary>
		/// Gets the kit groups
		/// </summary>
		public List<KitGroupData> Groups
		{
			get { return m_groups; }
			set { m_groups = value; }
		}
		...
		private void ApplyQuantityDiscountIfDefined(int quantity)
		{
			...
			if(customerLevelAllowsQuantityDiscounts)
			{
				...
				decimal qtyDiscount = QuantityDiscount.GetQuantityDiscountTablePercentageWithoutCartAwareness(ThisCustomer.CustomerID, this.Id, quantity, out qtyDiscountType);
		...
		public static KitProductData Find(int productId, int cartRecId, Customer thisCustomer, string locale)
		{
			...
			{
				dbconn.Open();
				using(var reader = DB.GetRS(String.Format("aspdnsf_getKitItems @ProductID={0}, @CartRecID={1}, @CustomerID={2}", productId, cartRecId, thisCustomer.CustomerID), dbconn))
				{
					while(reader.Read())
					{
			...
			if((cartRecId > 0) && (AppLogic.ShoppingCartRecIdIsValid(cartRecId)))
			{
			...
			kit.Groups = kit.Groups
				...
				.ThenBy(g => XmlCommon.GetLocaleEntry(g.Name, thisCustomer.LocaleSetting, true))
				.ToList();
			...
			foreach(var group in kit.Groups)
				group.Items = group.Items
					...
					.ThenBy(item => XmlCommon.GetLocaleEntry(item.Name, thisCustomer.LocaleSetting, true))
					.ToList();
	...
	public class KitGroupType
	{
		...
		{
			var all = new List<KitGroupType>();

			var query = "select KitGroupTypeId,Name from KitGroupType   with (NOLOCK)  order by DisplayOrder,Name";

			using(var conn = new SqlConnection(DB.GetDBConn()))
			{
				conn.Open();
				using(var rs = DB.GetRS(query, conn))
					while(rs.Read())
	...
	public class KitGroupData : IValidable
	{
		...
		public const int MULTI_SELECT_CHECKBOX_LIST = 3;
	...
	public class KitItemData : IValidable
	{
		...
		/// </summary>
		/// <param name="file"></param>
		public void UploadCustomerImage(HttpPostedFileBase file)
		{
			string extension = CommonLogic.ResolveExtensionFromMimeType(file.ContentType);
			string imgFileName = string.Empty;
		...
		/// </summary>
		/// <param name="file"></param>
		public void UploadImage(HttpPostedFileBase hpfile)
		{
		...
		private string SaveImage(string folder, string fileName, HttpPostedFileBase hpfile)
		{
	...
	public class ProductVariantForKit
	{
		...
		public static ProductVariantForKit Find(Customer ThisCustomer, int variantId)
		{
			...
			string unNamedVariant = "(Unnamed Variant)";
