    ...
    public class Feed
    {
        ...
        public string UpdateFeed(int StoreID, string FeedName, int DisplayOrder, string XmlPackage, short CanAutoFTP, string FTPUsername, string FTPPassword, string FTPServer, int FTPPort, string FTPFilename, string ExtensionData)
        {
            ...
            if (m_Feedid > 0)
            {
                ...
                if (StoreID == -1)
					m_SqlCmd.Parameters["@StoreID"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@StoreID"].Value = StoreID;

                if (FeedName == null)
					m_SqlCmd.Parameters["@Name"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@Name"].Value = FeedName;               

                if (DisplayOrder == -1)
					m_SqlCmd.Parameters["@DisplayOrder"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@DisplayOrder"].Value = DisplayOrder;

                if (XmlPackage == null)
					m_SqlCmd.Parameters["@XmlPackage"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@XmlPackage"].Value = XmlPackage;

                if (CanAutoFTP == -1)
					m_SqlCmd.Parameters["@CanAutoFTP"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@CanAutoFTP"].Value = CanAutoFTP;

                if (FTPUsername == null)
					m_SqlCmd.Parameters["@FTPUsername"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@FTPUsername"].Value = FTPUsername.Trim();

                if (FTPPassword == null)
					m_SqlCmd.Parameters["@FTPPassword"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@FTPPassword"].Value = FTPPassword.Trim();

                if (FTPServer == null)
					m_SqlCmd.Parameters["@FTPServer"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@FTPServer"].Value = FTPServer;

                if (FTPPort == -1)
					m_SqlCmd.Parameters["@FTPPort"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@FTPPort"].Value = FTPPort;

                if (FTPFilename == null)
					m_SqlCmd.Parameters["@FTPFilename"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@FTPFilename"].Value = FTPFilename.Trim();

                if (ExtensionData == null)
					m_SqlCmd.Parameters["@ExtensionData"].Value = DBNull.Value;
                else
					m_SqlCmd.Parameters["@ExtensionData"].Value = ExtensionData;
        ...
        public string ExecuteFeed(Customer ThisCustomer, String RuntimeParams)
        {
            ...
            try
            {
				...
				var HideProductsWithLessThanThisInventoryLevel = AppLogic.AppConfig("HideProductsWithLessThanThisInventoryLevel");
				...
				var SqlQuery = new StringBuilder(10000);
					...
					using(var cmd = new SqlCommand())
					{
						...
						try
						{
							...
							if(objReturn != null)
							{
								...
								var LowerBound = 1;
								var UpperBound = (LowerBound - 1) + 20000;
								...
								while(RowCount > 0)
								{
									...
									var feeddocument = AppLogic.RunXmlPackage(m_Xmlpackage, null, ThisCustomer, 1, "", RuntimeParams, false, false);
									...
									var filenamePrefix = LowerBound.ToString() + "_" + UpperBound.ToString();
									...
									if(this.CanAutoFTP)
									{
										var ftp = new FtpClient(this.FTPServer + ":" + this.FTPPort.ToString(), this.FTPUsername, this.FTPPassword);
