...
namespace AspDotNetStorefrontCore
{
	public class AvaTax
	{
		...
		{
            //If coming from address do not include Avalara-specific string resources
					...
					foreach (Message message in addressResult.Messages)
					{
                        if (message.Severity == SeverityLevel.Error)
                        {
                            if (stringResourceInResult)
                                errorMessages.Append(String.Format("<br />Avalara Address Validation Error: {0}", message.Details));
			...
			{
				return String.Format("<br />Avalara Address Validation failed: Exception {0}", ex.Message);
			}
		}
		...
		public void CommitTax(Order order)
		{
			...
			decimal totalAmount = Decimal.Round(order.SubTotal() + order.ShippingTotal(), 2, MidpointRounding.AwayFromZero);
		...
		private Line CreateLineItem(CartItem cartItem, Avalara.AvaTax.Adapter.AddressService.Address destinationAddress)
		{
			...
			else
			{
				...
				extendedPrice = Prices.LineItemPrice(cartItem, cartItems.CouponList, cartItem.ThisCustomer);
			}

			Line lineItem = new Line
			{
				ItemCode = cartItem.SKU,
				Description = FormatDescriptionForAvalara(cartItem.ProductName),
		...
		private IEnumerable<Line> CreateCartShippingLineItem(Avalara.AvaTax.Adapter.AddressService.Address destinationAddress, Customer customer, IEnumerable<CartItem> cartItems, IEnumerable<OrderOption> orderOptions)
		{
			...
			Line lineItem = new Line
			{
				...
				Description = FormatDescriptionForAvalara(string.Empty),
		...
		private IEnumerable<Line> CreateOrderOptionLineItems(IEnumerable<OrderOption> orderOptions, Avalara.AvaTax.Adapter.AddressService.Address destinationAddress)
		{
			return orderOptions.Select(oo => new
				...
				.Select(o => new Line
				{
					...
					Description = FormatDescriptionForAvalara(o.OrderOption.Name),
		...
		string FormatDescriptionForAvalara(string description)
		{
