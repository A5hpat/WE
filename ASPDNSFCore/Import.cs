...
namespace AspDotNetStorefrontCore
{
	...
	public class Import
    {
		static readonly ISearchEngineNameProvider SearchEngineNameProvider;
        ...
        private static void LoadCustomer(StringBuilder StatusList, XmlNode node)
        {
            ...
            else if (!Customer.NewEmailPassesDuplicationRules(nodeName, 0))
            {
            ...
            else
            {
                try
                {
                    ...
                    bool okToEmail = XmlCommon.XmlFieldBool(node, "OkToEMail");
                    if (XmlCommon.XmlField(node, "OkToEMail").Length == 0)
                    {
                        // something is wrong, just try to find the right case...
                        if (XmlCommon.XmlField(node, "OkToEmail").Length != 0)
                        {
							okToEmail = XmlCommon.XmlFieldBool(node, "OkToEmail");
                        }
                        if (XmlCommon.XmlField(node, "OKToEmail").Length != 0)
                        {
							okToEmail = XmlCommon.XmlFieldBool(node, "OKToEmail");
					...
					NewCustomer.UpdateCustomer(
						...
						okToEmail: okToEmail,
						billingEqualsShipping: CommonLogic.IIF(AppLogic.AppConfigBool("AllowShipToDifferentThanBillTo"), false, true),
						microPayBalance: XmlCommon.XmlFieldNativeDecimal(node, "MicropayBalance"),
						extensionData: XmlCommon.XmlField(node, "ExtensionData"),
						over13Checked: CommonLogic.IIF(XmlCommon.XmlFieldBool(node, "Over13Checked"), true, false),
						vatSetting: XmlCommon.XmlFieldUSInt(node, "VATSetting"),
						vatRegistrationId: XmlCommon.XmlField(node, "VATRegistrationID"),
						storeCreditCardInDb: CommonLogic.IIF(XmlCommon.XmlFieldBool(node, "StoreCCInDB"), true, false),
						isRegistered: CommonLogic.IIF(EMailField.Length != 0 && (PWD.Length != 0), true, false),
					...
					if (!AppLogic.AppConfigBool("VAT.Enabled") || !AppLogic.VATRegistrationIDIsValid(NewCustomer, vtr))
                    {
                        vtr = String.Empty;
                    }
        ...
        private static void LoadEntity(String EntityName, StringBuilder StatusList, XmlNode node)
        {
            ...
            else
            {
                ...
                if (!entityImportIsRestricted)
                {
                    ...
                    sql.Append(DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(nodeName), 100)) + ",");
        ...
        private static void LoadEntityTree(String EntityName, StringBuilder StatusList, XmlNode node)
        {
            ...
            foreach (var s in nodeName.Split('/'))
            {
				var PIC = CheckForEntity(EntityName, s, ParentEntityID);
                if (PIC != 0)
                {
                    ParentEntityID = PIC;
                }
                else
                {
                    ...
                    sql.Append(DB.SQuote(SearchEngineNameProvider.GenerateSeName(CommonLogic.Left(s, 100))));
                    sql.Append(")");
                    sql = sql.Replace("^", EntityName);
                    RunCommand(StatusList, sql.ToString());
                    ...
                    {
                        dbconn.Open();
                        using (var rs = DB.GetRS("select * from " + EntityName + "  with (NOLOCK)  where Deleted=0 and Name=" + DB.SQuote(s) + " and Parent" + EntityName + "ID=" + ParentEntityID.ToString(), dbconn))
                        {
                            rs.Read();
        ...
        private static void LoadProduct(StringBuilder StatusList, XmlNode node)
        {
			...
			var nodeName = XmlCommon.XmlField(node, "Name");
            ...
            else
            {
                ...
                {
                    dbconn.Open();
                    using (var rsp = DB.GetRS("select ProductID from dbo.Product with (NOLOCK) where Deleted=0 and ((len(SKU)>0 and SKU=" + DB.SQuote(XmlCommon.XmlField(node, "SKU")) + ") or (len(isnull(SKU,''))=0 and Name=" + DB.SQuote(nodeName) + "))", dbconn))
                    {
                        if (rsp.Read())
                        {
				...
				var WasJustAddedProduct = true;

				var TrackInventoryBySizeAndColor = (XmlCommon.XmlField(node, "TrackInventoryBySizeAndColor").Length != 0 && XmlCommon.XmlFieldBool(node, "TrackInventoryBySizeAndColor"));
                if (ProductID == 0)
                {
                    // add the product:
                    ...
                    sql.Append(DB.SQuote(CommonLogic.Left(nodeName, 400)) + ",");
                    sql.Append(DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(nodeName), 150)) + ",");
                    if (XmlCommon.XmlField(node, "ImageFilenameOverride").Length != 0)
                    {
                        sql.Append(DB.SQuote(XmlCommon.XmlField(node, "ImageFilenameOverride")) + ",");
                    }
                    ...
                    {
                        dbconn.Open();
                        using (var rs = DB.GetRS("select ProductID from Product  with (NOLOCK)  where Deleted=0 and ProductGUID=" + DB.SQuote(NewGUID), dbconn))
                        {
                            rs.Read();
					...
					var RelatedList = node.SelectNodes(@"RelatedProducts");
                ...
                else
                {
                    ...
                    sql.Append("SEName=" + DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(nodeName), 150)) + ",");
                    if (XmlCommon.XmlField(node, "ImageFilenameOverride").Length != 0)
                    {
        ...
        private static void LoadProductVariant(StringBuilder StatusList, XmlNode node, String ProductName, int ProductID, bool WasJustAddedProduct, bool TrackInventoryBySizeAndColor)
        {
            ...
            if (!WasJustAddedProduct)
            {
                ...
                {
                    dbconn.Open();
                    using (var rsp = DB.GetRS("select VariantID from ProductVariant  with (NOLOCK)  where Deleted=0 and ProductID=" + ProductID.ToString() + " and Name=" + DB.SQuote(nodeName) + CommonLogic.IIF(XmlCommon.XmlField(node, "SKUSuffix").Length != 0, " and SKUSuffix=" + DB.SQuote(XmlCommon.XmlField(node, "SKUSuffix")), " and (SKUSuffix='' or SKUSuffix IS NULL)"), dbconn))
                    {
                        if (rsp.Read())
                        {
            ...
            if (VariantID == 0)
            {
                ...
                {
                    dbconn.Open();
                    using (var rs = DB.GetRS("Select VariantID from ProductVariant where VariantGUID=" + DB.SQuote(NewGUID), dbconn))
                    {
                        if (rs.Read())
                        {
        ...
        private static void LoadKitGroup(StringBuilder StatusList, XmlNode node, String ProductName, int ProductID)
        {
            ...
            if (KitGroupID == 0)
            {
                ...
                {
                    dbconn.Open();
                    using (var rs = DB.GetRS("select KitGroupID from KitGroup  with (NOLOCK)  where KitGroupGUID=" + DB.SQuote(NewGUID), dbconn))
                    {
                        rs.Read();
		...
		public static int CheckForProductID(string EntityName, int ProductID)
        {
			var ID = 0;
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select ProductID from Product" + EntityName + " where ProductID = " + ProductID, dbconn))
                {
                    rs.Read();
        ...
        static public int CheckForEntity(String EntityName, String Name, bool AutoAdd)
        {
                ...
                foreach (String s in Name.Split('/'))
                {
                    ...
                    else
                    {
                        ...
                        if (!entityImportIsRestricted)
                        {
							var sql = new StringBuilder(2500);
							var NewGUID = CommonLogic.GetNewGUID();
							sql.Append(String.Format("insert {0} ({0}GUID,IsImport,Name,SEName,Parent{0}ID,XmlPackage) values(", EntityName));
                            sql.Append(DB.SQuote(NewGUID) + ",");
                            sql.Append("1,"); // Set IsImport Flag For later Undo of Import
                            sql.Append(DB.SQuote(CommonLogic.Left(s.Trim(), 400)) + ",");
                            sql.Append(DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(s.Trim()), 100)) + ",");
							sql.Append(ParentEntityID.ToString() + ",");
							sql.Append(DB.SQuote(AppLogic.ro_DefaultEntityXmlPackage));
                            sql.Append(")");
            ...
            else
            {
                ...
                {
                    dbconn.Open();
                    using (var rs = DB.GetRS("Select * from " + EntityName + "  with (NOLOCK)  where Parent" + EntityName + "ID=0 and Deleted=0 and lower(Name)=" + DB.SQuote(Name.Trim().ToLowerInvariant()), dbconn))
                    {
                        if (rs.Read())
                        {
                ...
                if (!entityImportIsRestricted)
                {
                    if (ID == 0 && AutoAdd)
                    {
                        ...
                        sql.Append(DB.SQuote(CommonLogic.Left(SearchEngineNameProvider.GenerateSeName(Name.Trim()), 100)));
                        sql.Append(")");
                        DB.ExecuteSQL(sql.ToString());
                        ...
                        {
                            dbconn.Open();
                            using (var rs = DB.GetRS("select * from " + EntityName + "  with (NOLOCK)  where Deleted=0 and Name=" + DB.SQuote(Name), dbconn))
                            {
                                rs.Read();
                                if (Name != DB.RSFieldByLocale(rs, EntityName, Localization.GetDefaultLocale()))
                                {
        ...
        static public int CheckForEntity(string EntityName, string Name, int ParentEntityID)
        {
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select * from " + EntityName + "  with (NOLOCK)  where Deleted=0 and Name LIKE '%" + Name.Replace("'", "''").Trim().ToLowerInvariant() + CommonLogic.IIF(ParentEntityID == 0, "%' and (Parent" + EntityName + "ID=0 or Parent" + EntityName + "ID IS NULL)", "%' and Parent" + EntityName + "ID=" + ParentEntityID.ToString()), dbconn))
                {
                    while (rs.Read())
                    {
        ...
        static public int CheckForProductType(string Name)
        {
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select ProductTypeID from ProductType  with (NOLOCK)  where lower(Name)=" + DB.SQuote(Name.Trim().ToLowerInvariant()), dbconn))
                {
                    if (rs.Read())
                    {
        ...
        static public int CheckForProduct(string Name)
        {
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select ProductID from Product  with (NOLOCK)  where Deleted=0 and lower(Name)=" + DB.SQuote(Name.Trim().ToLowerInvariant()), dbconn))
                {
                    if (rs.Read())
                    {
                        ID = DB.RSFieldInt(rs, "ProductID");
        ...
        static public int CheckForKitGroup(string Name, int ProductID)
        {
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select KitGroupID from KitGroup  with (NOLOCK)  where lower(Name)=" + DB.SQuote(Name.Trim().ToLowerInvariant()) + " and ProductID=" + ProductID.ToString(), dbconn))
                {
                    if (rs.Read())
                    {
        ...
        static public int CheckForKitItem(string Name, int KitGroupID)
        {
            ...
            {
                dbconn.Open();
                using (var rs = DB.GetRS("Select KitItemID from KitItem  with (NOLOCK)  where lower(Name)=" + DB.SQuote(Name.Trim().ToLowerInvariant()) + " and KitGroupID=" + KitGroupID.ToString(), dbconn))
                {
                    if (rs.Read())
                    {
